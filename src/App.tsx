...
function AppInner() {
  ...
  const [genreHistory, setGenreHistory] = useState<string[]>(() =>
    loadGenreHistory(),
  )
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/genre-descriptions.json')
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
      fetch('/genre-overrides.json')
  ...
  const advance = useCallback(
    (nextPrefs: Preferences, justSeen?: string) => {
      ...
      if (justSeen) {
        setGenreHistory(pushGenreHistory(justSeen))
      }
    },
    [
      genres,
  ...
  useEffect(() => {
    ...
    if (!genres.length || !Object.keys(simIndex).length || current) return
    advance(prefs)
  }, [
    genres,
    simIndex,
    ...
    body = (
      ...
      <header className="top">
        <div className="top-brand">
          ...
          <p className="sub">{modeLabel}</p>
        </div>
        <GenreSearch genres={genres} onSelect={openGenre} />
        <button
          type="button"
          className="icon-btn"
