...
export function GenreCard({
  ...
  async function ensureEnriched(g: Genre = local) {
    ...
    try {
      ...
      setLocal((prev) => {
        if (prev.artistsPinned) return prev
        return {
          ...prev,
          ...extra,
  ...
  async function saveGenrePlaylist() {
    ...
    const ok = await followPlaylist(playlistId)
  ...
  return (
    ...
    >
      <motion.article
        ...
        style={{ x, y, rotate, scale: cardScale, borderColor: local.color }}
        drag={!exiting}
      ...
      >
        <div className="card-scroll" ref={scrollRef}>
          <div
            className="hero"
            onPointerDown={(e) => {
              if (!exiting) dragControls.start(e)
            }}
          >
            <motion.img
              className="hero-img"
