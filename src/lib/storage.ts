...
const HISTORY_KEY = 'weirdnoise-genre-history-v1'
...
/** Last swiped genre ids, newest first (max 4). */
export function loadGenreHistory(): string[] {
  try {
...
export function pushGenreHistory(genreId: string): string[] {
  if (!genreId) return loadGenreHistory()
  const prev = loadGenreHistory().filter((id) => id !== genreId)
  const next = [genreId, ...prev].slice(0, HISTORY_LIMIT)
  try {
