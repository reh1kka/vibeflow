...
export async function fetchRealTopTracks(
  ...
  if (!best || bestScore < 40) best = items[0] ?? null
...
function demoTracks(artistId: string, name: string, seed: number): SpotifyTrack[] {
  return Array.from({ length: 5 }, (_, i) => {
    const title = `${pick(TRACK_WORDS, seed + i * 3)} ${pick(TRACK_WORDS, seed + i * 7 + 1)}`
    return {
      id: `demo-${artistId}-${i + 1}`,
      name: title,
      preview_url: null,
      duration_ms: 180000 + (seed % 90) * 1000,
      external_urls: {
        spotify: `https://open.spotify.com/search/${encodeURIComponent(`${name} ${title}`)}`,
      },
...
export async function buildLocalArtistBundle(
  ...
  const listeners = 120_000 + (seed % 4_800_000)
  ...
  const topTracks = deezerTop?.length
    ? deezerTop
    : demoTracks(artistId, name, seed)
