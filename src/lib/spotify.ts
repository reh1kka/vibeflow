...
export function loadUserAuth(): UserAuth | null {
  try {
    ...
    if (!raw) return null
    const parsed = JSON.parse(raw) as UserAuth & { isDemo?: boolean }
...
export async function fetchArtistBundle(
  ...
  if (userToken) {
    try {
      ...
      if (data.artist && data.topTracks.length) return data
    } catch {
      /* try proxy */
    }
  }
  ...
  try {
    ...
    if (res.ok && data.artist && data.topTracks?.length) {
      return { ...data, demo: false }
    }
    if (res.ok && data.artist) return { ...data, demo: false }
  } catch {
    // fall through
  }

  // 3) Local card (catalog + oEmbed), then one more proxy attempt
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.topTracks?.length) {
        return {
          ...local,
          artist: data.artist || local.artist,
...
async function spotifyUserPut(url: string): Promise<SpotifyWriteResult> {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      // Spotify rejects many PUTs without this header + body
...
/** Manual only — never called from swipes */
export async function saveTrack(trackId: string): Promise<SpotifyWriteResult> {
  if (trackId.startsWith('demo-') || trackId.startsWith('dz-')) {
    return { ok: false }
  }
  return spotifyUserPut(
...
export async function followArtist(
  ...
  return spotifyUserPut(
    `https://api.spotify.com/v1/me/following?type=artist&ids=${encodeURIComponent(artistId)}`,
  )
}

/** Manual only — follow/save playlist into library */
export async function followPlaylist(playlistId: string): Promise<boolean> {
  const token = await withUserToken()
...
export async function areTracksSaved(
  ...
  if (!token) return null
  const res = await fetch(
    `https://api.spotify.com/v1/me/tracks/contains?ids=${trackIds.map(encodeURIComponent).join(',')}`,
    { headers: { Authorization: `Bearer ${token}` } },
  )
  if (!res.ok) return null
...
export async function areArtistsFollowed(
  artistIds: string[],
): Promise<boolean[] | null> {
  const token = await withUserToken()
  if (!token) return null
  const res = await fetch(
    `https://api.spotify.com/v1/me/following/contains?type=artist&ids=${artistIds.map(encodeURIComponent).join(',')}`,
    { headers: { Authorization: `Bearer ${token}` } },
  )
  if (!res.ok) return null
  return (await res.json()) as boolean[]
