...
async function wikiFor(name, locale) {
  ...
  titles.add(`${titleCase(name)} (genre)`)
...
async function enrichPrimary(name, cache) {
  ...
  for (const locale of PRIMARY) {
    const cur = cache[name].byLang[locale]
    if (cur?.text && isWikiOrSeed(cur.source) && !RETRY) continue
    const hit = await wikiFor(name, locale)
    if (hit?.text) cache[name].byLang[locale] = hit
  }

  const en = cache[name].byLang.en
  if (!en?.text || isGenerated(en.source) || RETRY) {
    const lf = await lastFmTag(name)
    await sleep(DELAY)
    if (lf?.text) cache[name].byLang.en = lf
  }
...
async function main() {
  ...
  try {
    ...
    const pendingPrimary = names.filter((n) => {
      ...
      if (cache[n]?.triedPrimaryAt && real >= 1) return false
    ...
    await mapPool(pendingPrimary, CONCURRENCY, async (name) => {
      ...
      if (done % 25 === 0 || done === pendingPrimary.length || done <= 3) {
        ...
        log(
          `B ${done}/${pendingPrimary.length} wikiHits=${hits} genres=${s.ok} real=${s.real} langs=${s.langs}`,
        )
      }
    })
